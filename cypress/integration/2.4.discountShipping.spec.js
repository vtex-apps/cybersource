import {
  ...
  describe('Discount Shipping Testcase', () => {
    testSetup()
    const orderEnv = 'discountShipping1'
    ...
    it('Adding Product to Cart', updateRetry(3), () => {
        // Search the product
        cy.searchProduct(discountShipping.productName)
        // Add product to cart
        cy.addProduct(discountShipping.productName, { proceedtoCheckout: true})
    })

    it('Updating product quantity to 2', updateRetry(3), () => {
        // Update Product quantity to 2
        cy.updateProductQuantity(discountShipping, '2')
    })
    
    it('Verify tax and total', updateRetry(3), () => {
        // Verify Product tax and total
        cy.verifyTaxAndTotal(discountShipping)
    })
